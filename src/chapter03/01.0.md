...
```go
func t1() {
	a := [9]int{0, 1, 2, 3, 4, 5, 6, 7}
	fmt.Printf("  a:%v,%T,%d,%d", a, a, len(a), cap(a))
	b := a[1:4]
	fmt.Printf("  b:%v,%T,%d,%d", b, b, len(b), cap(b))
	c := b[2:6]
	fmt.Printf("  c:%v,%T,%d,%d", c, c, len(c), cap(c))
	d := a[:2][1:][:2]
	fmt.Printf("  d:%v,%T,%d,%d", d, d, len(d), cap(d))
	fmt.Println("address:", unsafe.Pointer(&a[3]), unsafe.Pointer(&b[2]), unsafe.Pointer(&c[0]), unsafe.Pointer(&d[1]))
	d[2] = 10
	fmt.Println(a, b, c, d)
}
执行结果：
 a:[0 1 2 3 4 5 6 7 0],[9]int,9,9  
 b:[1 2 3],[]int,3,8  
 c:[3 4 5 6],[]int,4,6  
 d:[1 2],[]int,2,8
 address: 0xc0000161a8 0xc0000161a8 0xc0000161a8 0xc0000161a8
 [0 1 2 3 10 5 6 7 0] [1 2 3] [3 10 5 6] [2 3 10]
```
同理如上，c在b的基础上截取2~6，显然是超出了b的长度，但未超出其容量，所以len会自动扩展。理解slice的底层是对数组的引用是关键,在a的基础上做切片操作，最后切片b,c,d的元素对应都是引用的底层数组的指针，从地址打印就能看出来。改了d[2]的值后，对应的数组及其他切片引用因为是引用指针，对应值也全部变化。
